...
class AnalyzerProvider:
    ...
    def provide_analyzer(self, plugin: str, data_type: InputDataType, node_type: NodeType, argument: str | None) -> GenericContextAnalyzer:
        if node_type == NodeType.AUTOMATIC:
            return AutomaticContextAnalyzer()
        if node_type == NodeType.MATCHING:
            return self.definitions.get_matching(plugin, data_type, argument).analyzer()
